...
class Bird(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        for num in range(1,4):
            img = pygame.image.load(f'imgs/bird{num}.png')
...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, x, y, position):
        ...
        self.rect = self.image.get_rect()
        # pos 1 is from the top else from the bottom
        if position == 1:
...
while run:
    ...
    # draw the ground
    screen.blit(ground_img, (ground_scroll, 621))
    ...
    if game_over == True and button.draw() == True:
        game_over = False
        score = reset_game()
